...
use App\UserFeed;
use App\UserDream;
use App\UserDreamComment;
use App\UserDreamLike;
...
class DreamController extends Controller
{
    public function __construct()
    {
        ...
        $this->main_module = 'dream';
        $this->module_type = config('custom_config.user_feed_type')['dreams'];
    }

    public function index(Request $request){
        $data = array('title' => 'Dreams','module'=>$this->main_module,'mainTable'=> 1,'likeTable'=>16,'commentTable'=>17);
        $data['dreamCount'] = UserFeed::whereHas('getUserDream')
                        ->select('id')->where('type_id','=',$this->module_type)
                        ->where('user_id','=',Auth::user()->id)
                        ->where('status','=','enable')
                        ->orderBy('id','DESC')->get();
        $data['results'] = UserFeed::whereHas('getUserDream')
                        ->where('type_id','=',$this->module_type)
                        ->where('user_id','=',Auth::user()->id)
                        ->where('status','=','enable')
                        ->orderBy('id','DESC')->offset(0)->limit(4)->get();
        $data['offset'] = 4;
        return view('dreams',$data);
    }

    public function add(){
        $data = array('title' => 'Add To Your Dream','method'=>'Add','action'=>route('dream.insert'),'frn_id'=>'frm_dream_add' ,'module'=>$this->main_module);
        return view('add_dream', $data);
    }
        ...
        try{
            ...
            if(!empty($file)){
                ...
                if($fileExtn == 'png' || $fileExtn == 'jpg' || $fileExtn == 'jpeg'){
                    ...
                    $tmp_path = config('custom_config.s3_dream_big');
                    ...
                    $tmp_path = config('custom_config.s3_dream_video');
                    $file_name = Common_function::fileNameTrimmer($file->hashName());
                    $path = $tmp_path.$file_name;
            ...
            UserDream::insert($insert_dream_array);
            ...
            if($save_draft == '' ){
                ...
                return redirect('/dream')->withSuccess(trans('custom.succ_dream_added'));
            }else{
                return redirect('/dream/edit/'.$feedId->id )->withSuccess(trans('custom.succ_dream_added_draft'));
            }
    ...
    public function edit($id){
        $data = array('title'=>" Edit Your Dream", 'method'=>'Edit','action'=>url('dream/update/'.$id),'frn_id'=>'frm_dream_edit' ,'module'=>$this->main_module);
            ...
            if(!empty($result)){
                ...
                if(!empty($file)){
                    ...
                    if($request->input('oldPhoto') != '' && $fileExtn != 'mp4' || $fileExtn != 'ogv' || $fileExtn != 'webm'){
                        Storage::disk('s3')->delete(config('custom_config.s3_dream_big').$request->input('oldPhoto'));
                        Storage::disk('s3')->delete(config('custom_config.s3_dream_thumb').$request->input('oldPhoto'));
                    }else{
                        Storage::disk('s3')->delete(config('custom_config.s3_dream_video').$request->input('oldPhoto'));
                    }

                    $update_dream_array['file'] = str_replace($tmp_path,'',$path);
                }
                $docfile = $request->file('uploaddoc');
                if(!empty($docfile)){
                    $insertDoc = array();
                ...
                UserDream::where('feed_id','=',$id)->update($update_dream_array);

                if($save_draft == '' ){
                    return redirect('/dream')->withSuccess(trans('custom.succ_dream_updated'));
                }else if($request->draft == 2){
                    Common_function::PostCreationEmail($this->module_type,$id,$this->main_module,'');
                    return redirect('/dream')->withSuccess(trans('custom.succ_dream_updated'));
                }else{
                    return redirect('/dream/edit/'.$id )->withSuccess(trans('custom.succ_dream_updated_draft'));
                }
            } else{
                return redirect()->route('dream.index');
            }
        }catch(\Illuminate\Database\QueryException $ex){ 
            return redirect()->back()->withInput();
        }
    ...
    public function loadMoreDream(Request $request){
        $output = array();
        ...
        {
            $html = view('include.dream-block',$data)->render();
    ...
    public function deleteFile(Request $request){
        ...
        if($request->file != '' && $extn[1] != 'mp4' || $extn[1] != 'ogv' || $extn[1] != 'webm'){
            Storage::disk('s3')->delete(config('custom_config.s3_dream_big').$request->file);
            Storage::disk('s3')->delete(config('custom_config.s3_dream_thumb').$request->file);
        }else{
            Storage::disk('s3')->delete(config('custom_config.s3_dream_video').$request->file);
        }
        $update_dreamfile_array['file'] = null;

        UserDream::where('feed_id','=',$request->id)->update($update_dreamfile_array);
        return response()->json(['code' => '1']);exit;
    }

    public function information($id){
        $data = array('title' => 'Dream Information','module'=>$this->main_module,'mainTable'=> 1,'likeTable'=>16,'commentTable'=>17);
        ...
        if(empty($data['results'])){
            return  redirect()->route('dream.index')->withError(trans('custom.error_delete_post'));
        }else{
            return view('dreams_detail',$data);
        }
    }
}
