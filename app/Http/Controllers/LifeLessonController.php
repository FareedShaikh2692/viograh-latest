...
use App\User;
use App\UserFeed;
use App\UserLifeLesson;
use App\UserLifeLessonComment;
use App\UserLifeLessonLike;
use App\FeedDocument;

class LifeLessonController extends Controller
{
    public function __construct()
    {
        $this->middleware('VerifyUser');
        $this->main_module = 'lifelesson';
        $this->module_type = config('custom_config.user_feed_type')['life_lessons'];
    }

    public function index(Request $request){
        $data = array('title' => 'Life Lessons','module'=>$this->main_module,'mainTable'=> 1,'likeTable'=>13,'commentTable'=>14);
        $data['lifeLessonCount'] = UserFeed::whereHas('getUserLifeLesson')
                        ->select('id')->where('type_id','=',$this->module_type)
                        ->where('user_id','=',Auth::user()->id)
                        ->where('status','=','enable')
                        ->orderBy('id','DESC')->get();
        $data['results'] = UserFeed::whereHas('getUserLifeLesson')
                        ->where('type_id','=',$this->module_type)
                        ->where('user_id','=',Auth::user()->id)
                        ->where('status','=','enable')
                        ->orderBy('id','DESC')->offset(0)->limit(4)->get();
        $data['offset'] = 4;
        return view('life_lessons',$data);
    }

    public function add(){
        $data = array('title' => 'Add To Your Life Lesson','method'=>'Add','action'=>route('lifelesson.insert'),'frn_id'=>'frm_lifelesson_add' ,'module'=>$this->main_module);
        return view('add_life_lesson', $data);
    }
    ...
    public function insert(Request $request){
        try{
            ...
            $validation_array = array(
                ...
                'uploaddoc.*' => 'mimes:jpeg,png,jpg,doc,pdf,xlsx,xls,ods',
            ...
            if(!empty($file)){
                ...
                if($fileExtn == 'png' || $fileExtn == 'jpg' || $fileExtn == 'jpeg'){
                    ...
                    $tmp_path = config('custom_config.s3_lifelesson_big');
                    ...
                    $tmp_path = config('custom_config.s3_lifelesson_video');
                    $file_name = Common_function::fileNameTrimmer($file->hashName());
                    $path = $tmp_path.$file_name;
            ...
            UserLifeLesson::insert($insert_lifelesson_array);
            ...
            if($save_draft == '' ){
                ...
                return redirect('/lifelesson')->withSuccess(trans('custom.succ_lifelesson_added'));
            }else{
                return redirect('/lifelesson/edit/'.$feedId->id )->withSuccess(trans('custom.succ_lifelesson_added_draft'));
            }
    ...
    public function edit($id){
        $data = array('title'=>" Edit  Your Life Lesson", 'method'=>'Edit','action'=>url('lifelesson/update/'.$id),'frn_id'=>'frm_lifelesson_edit' ,'module'=>$this->main_module);
            ...
            if(!empty($result)){
                ...
                if(!empty($file)){
                    ...
                    if($request->input('oldPhoto') != '' && $fileExtn != 'mp4' || $fileExtn != 'ogv' || $fileExtn != 'webm'){
                        Storage::disk('s3')->delete(config('custom_config.s3_lifelesson_big').$request->input('oldPhoto'));
                        Storage::disk('s3')->delete(config('custom_config.s3_lifelesson_thumb').$request->input('oldPhoto'));
                    }else{
                        Storage::disk('s3')->delete(config('custom_config.s3_lifelesson_video').$request->input('oldPhoto'));
                    }

                    $update_lifelesson_array['file'] = str_replace($tmp_path,'',$path);
                }
                $docfile = $request->file('uploaddoc');
                if(!empty($docfile)){
                    $insertDoc = array();
                ...
                UserLifeLesson::where('feed_id','=',$id)->update($update_lifelesson_array);
                if($save_draft == '' ){
                    return redirect('/lifelesson')->withSuccess(trans('custom.succ_lifelesson_updated'));
                }else if($request->draft == 2){
                    Common_function::PostCreationEmail($this->module_type,$id,$this->main_module,'');
                    return redirect('/lifelesson')->withSuccess(trans('custom.succ_lifelesson_updated'));
                }else{
                    return redirect('/lifelesson/edit/'.$id )->withSuccess(trans('custom.succ_lifelesson_updated_draft'));
                }
            } else{
                return redirect()->route('lifelesson.index');
            }
        }catch(\Illuminate\Database\QueryException $ex){ 
            return redirect()->back()->withInput();
        }
    ...
    public function loadMoreLifeLesson(Request $request){
        $output = array();
        ...
        {
            $html = view('include.lifelesson-block',$data)->render();
    ...
    public function deleteFile(Request $request){
        ...
        if($request->file != '' && $extn[1] != 'mp4' || $extn[1] != 'ogv' || $extn[1] != 'webm'){
            Storage::disk('s3')->delete(config('custom_config.s3_lifelesson_big').$request->file);
            Storage::disk('s3')->delete(config('custom_config.s3_lifelesson_thumb').$request->file);
        }else{
            Storage::disk('s3')->delete(config('custom_config.s3_lifelesson_video').$request->file);
        }
        $update_lifelessonfile_array['file'] = null;

        UserLifeLesson::where('feed_id','=',$request->id)->update($update_lifelessonfile_array);
        return response()->json(['code' => '1']);exit;
    }

    public function information($id){
        $data = array('title' => 'Lifelesson Information','module'=>$this->main_module,'mainTable'=> 1,'likeTable'=>13,'commentTable'=>14);
        ...
        if(empty($data['results'])){
            return  redirect()->route('lifelesson.index')->withError(trans('custom.error_delete_post'));
        }else{
            return view('life_lessons_detail',$data);
        }
    }
}
