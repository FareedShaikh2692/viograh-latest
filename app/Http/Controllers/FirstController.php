...
class FirstController extends Controller
{
    public function __construct()
    {
        ...
        $this->main_module = 'first';
        $this->module_type = config('custom_config.user_feed_type')['first_lasts'];
    }

    public function index(Request $request){
        $data = array('title' => 'Firsts','module'=>$this->main_module,'mainTable'=> 1,'likeTable'=>24,'commentTable'=>25);
        ...
        $data['results'] = UserFeed::whereHas('getUserFirst')
        ...
        $data['firstCount'] = UserFeed::whereHas('getUserFirst')
                        ->select('id')->where('type_id','=',$this->module_type)
                        ->where('status','=','enable')
                        ->where('user_id','=',Auth::user()->id)
                        ->whereIn('id',$feedIds)
                        ->orderBy('id','DESC')->get();
        return view('firsts',$data);
    }

    public function add(){
        $data = array('title' => 'Add To Your First','method'=>'Add','action'=>route('first.insert'),'frn_id'=>'frm_first_add' ,'module'=>$this->main_module);
        return view('add_first', $data);
    }
        ...
        try{
            ...
            if(!empty($file)){
                ...
                if($fileExtn == 'png' || $fileExtn == 'jpg' || $fileExtn == 'jpeg'){
                    ...
                    $tmp_path = config('custom_config.s3_first_big');
                    ...
                    $tmp_path = config('custom_config.s3_first_video');
                    $file_name = Common_function::fileNameTrimmer($file->hashName());
                    $path = $tmp_path.$file_name;
            ...
            UserFirstLast::insert($insert_first_array);
            ...
            if($save_draft == '' ){
                Common_function::PostCreationEmail($feedId->type_id,$feedId->id,$module,'first',$feedId->privacy);     
                return redirect('/first')->withSuccess(trans('custom.succ_first_added'));
            }else{
                return redirect('/first/edit/'.$feedId->id )->withSuccess(trans('custom.succ_first_added_draft'));
            }
    ...
    public function edit($id){
        $data = array('title'=>" Edit Your First", 'method'=>'Edit','action'=>url('first/update/'.$id),'frn_id'=>'frm_first_edit' ,'module'=>$this->main_module);
            ...
            if(!empty($result)){
                ...
                if(!empty($file)){
                    ...
                    if($request->input('oldPhoto') != '' && $fileExtn != 'mp4' || $fileExtn != 'ogv' || $fileExtn != 'webm'){
                        Storage::disk('s3')->delete(config('custom_config.s3_first_big').$request->input('oldPhoto'));
                        Storage::disk('s3')->delete(config('custom_config.s3_first_thumb').$request->input('oldPhoto'));
                    }else{
                        Storage::disk('s3')->delete(config('custom_config.s3_first_video').$request->input('oldPhoto'));
                    }
                    $update_first_array['file'] = str_replace($tmp_path,'',$path);
                }
                $docfile = $request->file('uploaddoc');
                if(!empty($docfile)){
                    $insertDoc = array();
                ...
                UserFirstLast::where('feed_id','=',$id)->update($update_first_array);
                if($save_draft == '' ){ 
                    return redirect('/first')->withSuccess(trans('custom.succ_first_updated'));
                }else if($request->draft == 2){
                    Common_function::PostCreationEmail($this->module_type,$id,$this->main_module,'first');
                    return redirect('/first')->withSuccess(trans('custom.succ_experience_updated'));
                }else{
                    return redirect('/first/edit/'.$id )->withSuccess(trans('custom.succ_first_updated_draft'));
                }
            } else{
    ...
    public function loadMoreFirst(Request $request){
        $output = array();
        ...
        {
            $html = view('include.first-block',$data)->render();
    ...
    public function deleteFile(Request $request){
        ...
        if($request->file != '' && $extn[1] != 'mp4' || $extn[1] != 'ogv' || $extn[1] != 'webm'){
            Storage::disk('s3')->delete(config('custom_config.s3_first_big').$request->file);
            Storage::disk('s3')->delete(config('custom_config.s3_first_thumb').$request->file);
        }else{
            Storage::disk('s3')->delete(config('custom_config.s3_first_video').$request->file);
        }
        $update_firstfile_array['file'] = null;

        UserFirstLast::where('feed_id','=',$request->id)->update($update_firstfile_array);
        return response()->json(['code' => '1']);exit;
    }

    public function information($id){
        $data = array('title' => 'First Information','module'=>$this->main_module,'mainTable'=> 1,'likeTable'=>24,'commentTable'=>25);
        ...
        if(empty($data['results'])){
            return  redirect()->route('first.index')->withError(trans('custom.error_delete_post'));            
        }else{
            return view('first_detail',$data);
        }
    }
}
