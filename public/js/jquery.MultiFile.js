...
if (window.jQuery)(function ($) {
	...
	$.fn.MultiFile = function (options) {
		...
		this
			...
			//.addClass('Multi').css('display','none')
			.each(function () {
				//#####################################################################
				...
				if (!(o.max > 0) /*IsNull(MultiFile.max)*/ ) {
					o.max = MultiFile.E.attr('data-maxlength');
				};
				if (!(o.max > 0) /*IsNull(MultiFile.max)*/ ) {
				...
				if (o.multiple !== false && o.multiple != undefined) {
				...
				MultiFile.E.wrap('<div class="MultiFile-wrap" id="' + MultiFile.wrapID + '_wrap"></div>');
				MultiFile.E.wrap('<div class="MultiFile-wrap" id="' + MultiFile.wrapID + '_wrap"></div>');
				MultiFile.wrapper = $('#' + MultiFile.wrapID + '_wrap');
				...
				if (!MultiFile.list) {
					...
					MultiFile.wrapper.closest('.upload_doc_box').after('<div class="MultiFile-list" id="' + MultiFile.wrapID + '_list"></div>');
				...
				MultiFile.addSlave = function (slave, slave_count) {
					...
					slave.change(function (a, b, c) {
						...
						//if($(this).attr('data-maxlength') != 0){
								...
								var v = file.name.replace(/^C:\\fakepath\\/gi,''),
										...
										p = function(z){
											return z
												...
												.replace('$size', sl(s)) //.replace('$size', sl(s) + ' > ' + sl(MultiFile.maxfile))
										}
								;
							...
							newfs.total_length = newfs.length + $('.MultiFile-label-doc').length; //prevs.length;
							
							// limit the number of files selected
							if (MultiFile.max>0 && $('.MultiFile-label-doc').length + files.length > MultiFile.max) {
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					/*$.each(files, function (i, file) {
						var v = String(file.name || '' ).replace(/[&<>'"]/g, function(c) { return '&#'+c.charCodeAt()+';'; }),
								...
								label =	$(
										(
											'<a href="javascript:;" class="doc-multi-js">'+
					...
					$.each(files, function (i, file) {
						...
						label =	$(
								(
									...
									'<span class="MultiFile-label" title="">'+
										'<span class="MultiFile-title multifile-name">'+ n +'</span>'+
										(MultiFile.preview || $(slave).is('.with-preview') ? p : '' )+
									'</span>'+'</a>'
								)
								.replace(/\$(file|name)/gi, (v.match(/[^\/\\]+$/gi)||[v])[0])
						...
						var
							r = $('<div class="MultiFile-label MultiFile-label-doc" ><img class="pinimage" src="'+baseUrl+'/images/icons/link.svg" class="img-fluid" alt="VioGraf" ></div>'),
							b = $('<a class="MultiFile-remove"  data-index="'+i+'" href="#' + MultiFile.wrapID + '">' + MultiFile.STRING.remove + '</a>')
								...
								.click(function () {
									...
									var files_being_removed = FILE_LIST(slave);
								 	console.log(files_being_removed);
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		accept: 'gif|jpg|jpeg|png|xlsx|xls|ods|pdf|txt|doc|docx', // accepted file extensions
		max: 5, // maximum number of selectable files
		maxfile: 10240, // maximum size of a single file
		//maxsize: 10240, // maximum size of entire payload
		...
		STRING: {
			remove: '<i class="far fa-times-circle close-doc"></i>',
			denied: 'Only  JPEG, JPG, PNG, PDF, DOC, DOCX, XLS, XLSX, TXT and ODS  files are allowed.<br>',//'You cannot select a $ext file.\nTry again...<br>',
			file: '$file',
			selected: 'File selected: $file.<br>',
			duplicate: 'This file :\n$file has already been selected.<br>',
			toomuch: 'The files selected exceed the maximum size permited ($size).<br>',
			toomany: 'Maximum 5 documents are allowed.<br> ',//'MaToo many files selected (max: $max)',
			toobig: '$file Size  $size > Max size 10.0 MB.<br>'
		},
		...
		error: function (s) {			
			...
			$(".upload_doc_box").removeClass("doc-error");
			$(".upload_doc_box").removeClass("has-error");
			$(".doc-error").remove();
			$(".upload_doc_box").addClass("has-error");
			$('#uploaddoc-error').text('');

			//$('.upload_doc_box').after('<label id="uploaddoc-error" class="help-block doc-error">'+s.toString()+'</label>');
			$('.upload_doc_box').after('<label id="uploaddoc-error" class="help-block doc-error"></label>');
			$('#uploaddoc-error').html(s);
