@foreach($results as $key=>$row)
    <li class="col-lg-3 col-md-3 col-sm-4  col-6 viograf_col">
        <a href="{{ url('/first/information/'.$row->id) }}" class="inner_diary_box">
                @php
                    @$ext = explode(".",@$row->getUserFirst->file);
                @endphp
                @if(@$ext[1] == 'mp4' || @$ext[1] == 'ogv' || @$ext[1] == 'webm')
                    <div class="viografbox_img">
                        <video><source id="videoSource"/ src="{{ Common_function::get_s3_file(config('custom_config.s3_first_video'),$row->getUserFirst->file) }}"></video>
                    </div>
                @else
                    <div class="viografbox_img feedImage" style="background-image:url('{{@$row->getUserFirst->file ? Common_function::get_s3_file(config('custom_config.s3_first_thumb'),@$row->getUserFirst->file) : url('images/default/firstlast_default.jpg')}}')">
                    </div>
                @endif
