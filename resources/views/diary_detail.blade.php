    ...
    <div class="post-detail-page">
        <div class="inner_back_center_row row">
            <div class="inner_back_col">
                <a href="{{ (Common_function::previous_url()[0][3] == strtolower(@$module)) ? route('diary.index') : Common_function::previous_url()[1] }}" class="btn_back">
                    ...
                    <div class="education-imgs-lider slider">
                        <div class="banner_box">
                            <div class="banner_img">                       
                                @php
                                    @$ext = explode(".",@$results->getUserDiary->file);                             
                                @endphp
                                @if(@$results->getUserDiary->file != '')
                                    @if(@$ext[1] == 'mp4' || @$ext[1] == 'ogv' || @$ext[1] == 'webm' ) 
                                    <div class="video_box"></div>                           
                                        <video controls class="img-fluid" disablepictureinpicture controlslist="nodownload"><source id="videoSource"/ src="{{Common_function::get_s3_file(config('custom_config.s3_diary_video'),@$results->getUserDiary->file)}}"></video>
                                    @else
                                        <img src="{{ Common_function::get_s3_file(config('custom_config.s3_diary_big'),@$results->getUserDiary->file) }}" class="img-fluid"
                                            alt="VioGraf" />
                                    @endif
                                @else 
                                    <img src="{{asset('images/default/diary_default.png')}}" class="img-fluid" alt="VioGraf" />   
                                @endif
                    ...
                    <div class="post-detail-area">
                        <div class="post_title_row">
                            <div class="post_title_box">
                                <h1 class="main_title">
                                    <i style="color:black;font-size:14px;">{{ ($results->getUserDiary->date != '') ? date("jS F, Y", strtotime($results->getUserDiary->date)) : '-'}}</i>
                                </h1>
                            </div>
                            @if(@$results->user_id == Auth::user()->id)
                                <div class="edit_delete_icon">
                                    <a href="{{url('diary/edit/'.$results->id)}}" title="Edit">
                        ...
                        <div class="post_detail_box">
                            ...
                            <div class="detail_info">
                                <h3>Description</h3>        
                                <h4><span class="desc-info">{!!(@$results->getUserDiary->description != '') ? Common_function::string_read_more(@$results->getUserDiary->description) : '-' !!}</span></h4>
                            </div>
                        </div>
                        @if(@$results->is_save_as_draft == 0)
                            <div>
                                <ul class="like_comments_row">
                                    <li class="like_box">
                                        <button type="button" class="like_unlike {{(@$results->getUserDiaryLike->is_like != 0 )? 'active':''}}" id="like_unlike" value=""><i class="fa fa-heart" aria-hidden="true"></i>
                            ...
                            <div class="comment_area">
                                ...
                                <ul class="comment_list">
                                    <div class="cmts">
                                        @foreach (@$results->getUserDiaryComment as $k => $val)
                                            <li>
                                                <div class="img_col">
                                                    <img src="{{  @$val->getDiaryCommentedUser->profile_image ? Common_function::get_s3_file(config('custom_config.s3_user_big'),  @$val->getDiaryCommentedUser->profile_image) : url('images/userdefault-1.svg') }}" alt="Family Feed" class="img-fluid">
                                                </div>
                                                <a href="{{route('public-profile.index',@$val->getDiaryCommentedUser->id)}}">
                                                    <div class="commentdetail">
                                                        <h4>{{@$val->getDiaryCommentedUser->first_name.' '.@$val->getDiaryCommentedUser->last_name}}  <p class="commentTime">&nbsp;{{Common_function::get_time_ago(strtotime($val->created_at))}}</p></h4>
                                                        <p>{{@$val->comment}}</p>
                                                    </div>
                                                </a>
                                            </li>
