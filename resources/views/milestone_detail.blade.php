    ...
    <div class="post-detail-page">
        <div class="inner_back_center_row row">
            <div class="inner_back_col">
                <a href="{{ (Common_function::previous_url()[0][3] == strtolower(@$module)) ? route('milestone.index') : Common_function::previous_url()[1] }}" class="btn_back">
                    ...
                    <div class="education-imgs-lider slider">
                        <div class="banner_box">
                            <div class="banner_img">                       
                                @php
                                    @$ext = explode(".",@$results->getUserMilestone->file);                             
                                @endphp
                                @if(@$results->getUserMilestone->file != '')
                                    @if(@$ext[1] == 'mp4' || @$ext[1] == 'ogv' || @$ext[1] == 'webm' ) 
                                    <div class="video_box"></div>                           
                                        <video controls class="img-fluid" disablepictureinpicture controlslist="nodownload"><source id="videoSource"/ src="{{Common_function::get_s3_file(config('custom_config.s3_milestone_video'),@$results->getUserMilestone->file)}}"></video>
                                    @else
                                        <img src="{{ Common_function::get_s3_file(config('custom_config.s3_milestone_big'),@$results->getUserMilestone->file) }}" class="img-fluid"
                                            alt="VioGraf" />
                                    @endif
                                @else 
                                    <img src="{{asset('images/default/milestone_default.jpg')}}" class="img-fluid" alt="VioGraf" />   
                                @endif
                    ...
                    <div class="post-detail-area">
                        <div class="post_title_row">
                            <div class="post_title_box">
                                <h1 class="main_title">{{(@$results->getUserMilestone->title !='') ? @$results->getUserMilestone->title : '-'}}
                            ...
                            @if(@$results->user_id == Auth::user()->id)
                                <div class="edit_delete_icon">
                                    <a href="{{ route('milestone.edit',$results->id) }}" title="Edit">
                        ...
                        <div class="post_detail_box">
                            <div class="detail_info"> 
                                <h3>Milestone Completion Date</h3>                            
                                <h4><span>{{(@$results->getUserMilestone->achieve_date != '') ? date('D d, M Y',strtotime(@$results->getUserMilestone->achieve_date)) : '-'}}</span></h4>
                            </div>
                            ...
                            <div class="detail_info">
                                <h3>Description</h3>        
                                <h4><span class="desc-info">{!! (@$results->getUserMilestone->description != '') ? Common_function::string_read_more(@$results->getUserMilestone->description) : '-' !!}</span></h4>
                            </div>
                        </div>
                        @if(@$results->is_save_as_draft == 0)
                            <div>
                                <ul class="like_comments_row">
                                    <li class="like_box">
                                        <button type="button" class="like_unlike {{(@$results->getUserMilestoneLike->is_like != 0 )? 'active':''}}" id="like_unlike" value=""><i class="fa fa-heart" aria-hidden="true"></i>
                            ...
                            <div class="comment_area">
                                ...
                                <ul class="comment_list">
                                    <div class="cmts">
                                        @foreach (@$results->getUserMilestoneComment as $k => $val)
                                            <li>
                                                <div class="img_col">
                                                    <img src="{{  @$val->getMilestoneCommentedUser->profile_image ? Common_function::get_s3_file(config('custom_config.s3_user_big'),  @$val->getMilestoneCommentedUser->profile_image) : url('images/userdefault-1.svg') }}" alt="Family Feed" class="img-fluid">
                                                </div>
                                                <a href="{{route('public-profile.index',@$val->getMilestoneCommentedUser->id)}}">
                                                    <div class="commentdetail">
                                                        <h4>{{@$val->getMilestoneCommentedUser->first_name.' '.@$val->getMilestoneCommentedUser->last_name}}  <p class="commentTime">&nbsp;{{Common_function::get_time_ago(strtotime($val->created_at))}}</p></h4>
                                                        <p>{{@$val->comment}}</p>
                                                    </div>
                                                </a>
                                            </li>
                                    ...
                                    @if(@$results->comment_count > 3)
                                        <div class="view_more_commentsarea" >
                                            <div id="view_more_commentsarea">
                                                <button type="button" class="btn-more" id="btn-more" data-cmtcount="{{count(@$results->getUserDiaryComment)}}" data-id ="{{@$val->id}}">View more comments</button>
                                            </div>
                                            <span> {{count(@$results->getUserDiaryComment)}} of {{Common_function::like_comment_count(@$results->comment_count)}}</span>
                                        </div>
                                    @endif
                                </ul>
                            </div>
