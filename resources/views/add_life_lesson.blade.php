...
<div class="welcome-familyarea">
        <div class="inner_back_center_row row">
            <div class="inner_back_col">
                <a href="javascript:;" data-url="{{route('lifelesson.index')}}" class="btn_back">
                        ...
                        <form method="post" action="{{@$action}}" id="{{@$frn_id}}" class="form {{(@$method == 'Add' ? 'js-draft-form' : '')}}" enctype="multipart/form-data">
                            ...
                            <div class="form-group form_box upload_photo">
                                ...
                                <div class="upload_photo_box upload_photo_video">
                                    ...
                                    <input type="hidden" name="oldPhoto" id="oldPhoto" data-id="{{@$result->getUserLifeLesson->feed_id}}" data-url="delete-lifelessonfile" value="{{@$result->getUserLifeLesson->file}}">
                                </div>
                                <div class="upload_show">
                                    <ul class="addimgs_area" >
                                        @if(@$method != 'Add' && $result->getUserLifeLesson->file != '')
                                            @php
                                                $extn = explode(".",@$result->getUserLifeLesson->file);
                                            @endphp
                                            @if ($extn[1] == 'mp4' || @$extn[1] == 'ogv' || @$extn[1] == 'webm')
                                                <li class="addimg"> 
                                                <button type="button" class="close-btn"> <img src="{{asset('images/icons/upload_file_close.svg')}}" alt="Vio Graf" /></button> <video><source id="videoSource"/ src="{{Common_function::get_s3_file(config('custom_config.s3_lifelesson_video'),$result->getUserLifeLesson->file)}}"></video>
                                                </li>
                                            @else
                                                <li class="addimg"> 
                                                <button type="button" class="close-btn"> <img src="{{asset('images/icons/upload_file_close.svg')}}" alt="Vio Graf" /></button>  <img src="{{Common_function::get_s3_file(config('custom_config.s3_lifelesson_thumb'),$result->getUserLifeLesson->file)}}" alt="Vio Graf" /> 
                                                </li>
                                            @endif
                                        @endif
                                    </ul>
                            ...
                            <div class="form-group form_box">
                                <p>Title</p>
                                <input type="text"  name="title" id="title" value="{{old('title',@$result->getUserLifeLesson->title)}}" class="form-control required">
                            </div>
                            <div class="form-group form_box">
                                <p>Description</p>
                                <textarea class="form-control form_textarea" name="description" id="description" placeholder="" >{{old('description',@$result->getUserLifeLesson->description)}}</textarea>
                            </div>
                            <div class="form-group form_box upload_photo">
