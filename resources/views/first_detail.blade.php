    ...
    <div class="post-detail-page">
        <div class="inner_back_center_row row">
            <div class="inner_back_col">
                <a href="{{ (Common_function::previous_url()[0][3] == strtolower(@$module)) ? route('first.index') : Common_function::previous_url()[1] }}" class="btn_back">
                    ...
                    <div class="education-imgs-lider slider">
                        <div class="banner_box">
                            <div class="banner_img">                       
                                @php
                                    @$ext = explode(".",@$results->getUserFirst->file);                             
                                @endphp
                                @if(@$results->getUserFirst->file != '')
                                    @if(@$ext[1] == 'mp4' || @$ext[1] == 'ogv' || @$ext[1] == 'webm') 
                                    <div class="video_box"></div>                           
                                        <video controls class="img-fluid" disablepictureinpicture controlslist="nodownload"><source id="videoSource"/ src="{{@$results->getUserFirst->file ? Common_function::get_s3_file(config('custom_config.s3_first_video'),@$results->getUserFirst->file) : asset('images/default_banner.jpg')}}"></video>
                                    @else
                                        <img src="{{ @$results->getUserFirst->file ? Common_function::get_s3_file(config('custom_config.s3_first_big'),@$results->getUserFirst->file) : asset('images/default/firstlast_default.jpg') }}" class="img-fluid"
                                            alt="VioGraf" />
                                    @endif
                                @else 
                    ...
                    <div class="post-detail-area">
                        ...
                        <div class="post_detail_box">
                            ...
                            <div class="detail_info">
                                <h3>Description</h3>        
                                <h4><span class="desc-info">{!! (@$results->getUserFirst->description != '') ? Common_function::string_read_more(@$results->getUserFirst->description) : '-'  !!}</span></h4>
                            </div>
                        </div>
                        @if(@$results->is_save_as_draft == 0)
                            <div>
                                <ul class="like_comments_row">
                                    <li class="like_box">
                                        <button type="button" class="like_unlike {{(@$results->getUserFirstLike->is_like != 0 )? 'active':''}}" id="like_unlike" value=""><i class="fa fa-heart" aria-hidden="true"></i>
                            ...
                            <div class="comment_area">
                                ...
                                <ul class="comment_list">
                                    <div class="cmts">
                                        @foreach(@$results->getUserFirstComment as $k => $val)
                                            <li>
                                                <div class="img_col">
                                    ...
                                    @if(@$results->comment_count > 3)
                                        <div class="view_more_commentsarea" >
                                            <div id="view_more_commentsarea">
                                                <button type="button" class="btn-more" id="btn-more" data-cmtcount="{{count(@$results->getUserFirstComment)}}" data-id ="{{@$val->id}}">View more comments</button>
                                            </div>
                                            <span>{{count(@$results->getUserFirstComment)}} of {{(Common_function::like_comment_count(@$results->comment_count))}}</span>
                                        </div>
                                    @endif
                                </ul>
                            </div>
