                 ...
                 <div class="inner_center_page">
                  ...
                  <div class="form-group form_box upload_photo">
                    ...
                    <div class="upload_photo_box upload_doc_box">
                      <input type="file"  class="upload_photobtn js-multi-file" name="uploaddoc[]"  id="uploaddoc" data-maxlength="{{ ($method == 'Edit') ? 5 - @$result->getUserLiabilityDocuments->count() : 5}}" accept="gif|jpg|jpeg|png|xlsx|xls|ods|pdf|txt|doc|docx" maxsize=1024>
                      <input type="hidden" name="oldPhoto" id="oldPhoto"  data-url="delete-assetfile" value="">
                  </div>
                  @if ($method == 'Edit')
                                    <div class="filediv mt-4">
                                    @foreach ($result->getUserLiabilityDocuments as $dockey => $docval)
                                    <div id="doc_{{$docval->id}}" class="row MultiFile-label-doc">
                                        <a href="{{Common_function::get_s3_file(str_replace('[id]',$result->id,config('custom_config.s3_liability_document')),$docval->file)}}" target="_blank" class=" docfiles text-truncate" download>
                                        <span><img src="{{asset('images/icons/link.svg')}}" class="img-fluid" alt="VioGraf"></span>{{@$docval->file}}</a><button type="button" id="js-close-remove-file-liability" class="deletedoc js-close-remove-file-asset" name="deletedocs" data-file="{{$docval->file}}" data-id="{{$docval->id}}" data-feed="{{$result->id}}"><i class="far fa-times-circle"></i></button><p class="col-xl-6 col-lg-6 col-md-4 col-sm-3"></p>
                                    </div>
                                    @endforeach
                                    </div>
                                @endif
                          ...
                          <div class="row">
                              ...
                              <div class="form-group form_box">
                                <p>Amount<span class="span_extension_note">{{trans('custom.asset_note')}}</span></p>
                                  <input type="text" name="amount" class="form-control required amount"   value="{{old('amount',@$result->amount)}}">
                                </div>  
                              </div>
                            </div> 
                            <div class="form-group form_box">
                            ...
                            <div class="row">
                              <div class="col-md-12">
                                <h3 class="sub_title mt-4">Bank Detail</h3>
                              </div>
                              <div class="col-md-6">
                                <div class="form-group form_box" id="bankdetails">
                                  <p>Bank Name</p>
                                  <input type="text" name="bank_name" id="bank_name" class="form-control" value="{{old('bank_name',@$result->bank_name)}}">
                                </div>
                              </div>
                              <div class="col-md-6">
                                <div class="form-group form_box">
                                  <p>Account number</p>
                                  <input type="text" name="account_number" id="account_number" class="form-control" value="{{old('account_number',@$result->account_number)}}">
                                </div>
                              </div> 
                            </div>
                          ...
                          <div class="dropdownsubmit_row">
                              ...
                              <div class="dropdown_col public-dropdown">
                              </div>
...
@section('js')
   <!--// plugin-specific resources for upload docs //-->
