...
@section('body')
    <div class="wrapper">
        ...
        <nav class="main-header navbar {{config('adminlte.classes_topnav_nav', 'navbar-expand-md')}} {{config('adminlte.classes_topnav', 'navbar-white navbar-light')}}">
            <div class="{{config('adminlte.classes_topnav_container', 'container')}}">
                    ...
                    <a href="{{ $dashboard_url }}" class="navbar-brand {{ config('adminlte.classes_brand') }}">
                        <img src="{{ asset(config('adminlte.logo_img', 'vendor/adminlte/dist/img/AdminLTELogo.png')) }}" alt="{{config('adminlte.logo_img_alt', 'AdminLTE')}}" class="{{ config('adminlte.logo_img_class', 'brand-image elevation-3') }}" style="opacity: .8">
                    ...
                    @if(Auth::guard('manage'))
                        ...
                        <li class="nav-item dropdown user-menu">
                            <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">
                                ...
                                <img src="{{@Auth::guard('manage')->user()->profile_photo? Common_function::get_s3_file(config('custom_config.s3_admin_thumb'),Auth::guard('manage')->user()->profile_photo) : url('images/default.png') }}" class="user-image img-circle elevation-2" alt="{{ Auth::guard('manage')->user()->first_name.' '.Auth::guard('manage')->user()->last_name }}">
                                @endif
                                <span @if(config('adminlte.usermenu_image'))class="d-none d-md-inline"@endif>{{ Auth::guard('manage')->user()->first_name.' '.Auth::guard('manage')->user()->last_name }}</span>
                            </a>
                            <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                                @if(!View::hasSection('usermenu_header') && config('adminlte.usermenu_header'))
                                <li class="user-header {{ config('adminlte.usermenu_header_class', 'bg-primary') }}  h-auto ">
                                    @if(config('adminlte.usermenu_image'))
                                    <img src="{{Auth::guard('manage')->user()->profile_photo? Common_function::get_s3_file(config('custom_config.s3_admin_thumb'),Auth::guard('manage')->user()->profile_photo) : url('images/default.png')  }}" class="img-circle elevation-2" alt="{{ Auth::guard('manage')->user()->first_name.' '.Auth::guard('manage')->user()->last_name }}">
                                    @endif
                                    <p class="@if(!config('adminlte.usermenu_image'))mt-0 @endif">
                                        {{ Auth::guard('manage')->user()->first_name.' '.Auth::guard('manage')->user()->last_name }}
        ...
        @if(!config('adminlte.layout_topnav') && !View::getSection('layout_topnav'))
        <aside class="main-sidebar {{config('adminlte.classes_sidebar', 'sidebar-dark-primary')}}">
            @if(config('adminlte.logo_img_xl'))
                <a href="{{ $dashboard_url }}" class="brand-link logo-switch" style="padding-bottom:9px;">
                    <img src="{{ asset(config('adminlte.logo_img_xl', 'vendor/adminlte/dist/img/AdminLTELogo.png')) }}" alt="{{config('adminlte.logo_img_alt', 'AdminLTE')}}" class="{{config('adminlte.logo_img_class', 'brand-image')}} logo-xs">
                    <img src="{{ asset(config('adminlte.logo_img')) }}" alt="{{config('adminlte.logo_img_alt', 'AdminLTE')}}" class="{{config('adminlte.logo_img_xl_class', 'brand-image-xs')}} logo-xl">
                    <!--APP NAME IN SIDEBAR-->
                    <span style="font-size:22px;" class="brand-text font-weight-light {{ config('adminlte.classes_brand_text') }}">
                        {!! config('adminlte.logo', '<b>Admin</b>LTE') !!}
                    </span>
                </a>
            @else
                <a href="{{ $dashboard_url }}" class="brand-link {{ config('adminlte.classes_brand') }}">
                    <img src="{{ asset(config('adminlte.logo_img', 'vendor/adminlte/dist/img/AdminLTELogo.png')) }}" alt="{{config('adminlte.logo_img_alt', 'AdminLTE')}}" class="{{ config('adminlte.logo_img_class', 'brand-image img-circle') }}" style="opacity: .8">
                    <!--ALLSTATE APPLIANCE APP NAME IN SIDEBAR-->
                    <span style="font-size:22px;" class="brand-text font-weight-light {{ config('adminlte.classes_brand_text') }}">
                        {!! config('adminlte.logo', '<b>Admin</b>LTE') !!}
                    </span>
                </a>
...
@section('adminlte_js')
    <script src="{{ asset('js/manage/jquery.fancybox.js') }}"></script>
    <script src="{{ asset('js/manage/select2.js') }}"></script>
    <script src="{{ asset('js/manage/pace.min.js') }}"></script>
    <script src="{{ asset('vendor/adminlte/dist/js/adminlte.min.js') }}"></script>
    <script type="text/javascript">
        var siteUrl= "{{url('/manage')}}";
        var baseUrl= "{{url('/')}}";
    </script>
    <script src="https://cdn.ckeditor.com/4.14.1/standard/ckeditor.js"></script>
    <script src="{{ asset('js/manage/core.js') }}"></script>
    <script src="{{ asset('js/manage/fullscreen.js') }}"></script>
    <script src="{{ asset('js/manage/guestures.js') }}"></script>
    <script src="{{ asset('js/manage/hash.js') }}"></script>
    <script src="{{ asset('js/manage/media.js') }}"></script>
    <script src="{{ asset('js/manage/share.js') }}"></script>
    <script src="{{ asset('js/manage/slideshow.js') }}"></script>
    <script src="{{ asset('js/manage/thumbs.js') }}"></script>
    <script src="{{ asset('js/manage/wheel.js') }}"></script>
    <script src="{{ asset('js/manage/bootstrap-notify.min.js') }}"></script>
