    ...
    <div class="welcome-familyarea">
        <div class="inner_back_center_row row">
            <div class="inner_back_col">
                <a href="javascript:;" data-url="{{route('experience.index')}}" class="btn_back">
                        ...
                        <form method="post" action="{{@$action}}" id="{{@$frn_id}}" class="form {{(@$method == 'Add' ? 'js-draft-form' : '')}}" enctype="multipart/form-data">
                            ...
                            <div class="form-group form_box upload_photo">
                                ...
                                <div class="upload_photo_box upload_photo_video">
                                    ...
                                    <input type="hidden" name="oldPhoto" id="oldPhoto" data-id="{{@$result->getUserExperience->feed_id}}" data-url="delete-experiencefile" value="{{@$result->getUserExperience->file}}">
                                </div>
                                <div class="upload_show">
                                    <ul class="addimgs_area" >
                                        @if(@$method != 'Add' && @$result->getUserExperience->file != '')
                                            @php
                                                $extn = explode(".",@$result->getUserExperience->file);
                                            @endphp
                                            @if ($extn[1] == 'mp4' || @$extn[1] == 'ogv' || @$extn[1] == 'webm')
                                                <li class="addimg"> 
                                                <button type="button" class="close-btn"> <img src="{{asset('images/icons/upload_file_close.svg')}}" alt="Vio Graf" /></button> <video><source id="videoSource"/ src="{{Common_function::get_s3_file(config('custom_config.s3_experience_video'),$result->getUserExperience->file)}}"></video>
                                                </li>
                                            @else
                                                <li class="addimg"> 
                                                <button type="button" class="close-btn"> <img src="{{asset('images/icons/upload_file_close.svg')}}" alt="Vio Graf" /></button>  <img src="{{Common_function::get_s3_file(config('custom_config.s3_experience_thumb'),$result->getUserExperience->file)}}" alt="Vio Graf" /> 
                                                </li>
                                            @endif
                                        @endif
                                    </ul>
                            ...
                            <div class="form-group form_box">
                                <p>Title</p>
                                <input type="text" name="title" id="title" value="{{old('title',@$result->getUserExperience->title)}}" class="form-control">
                            </div>
                            <div class="row">
                                <div class="col-lg-4">
                                    <div class="form-group form_box">
                                        ...
                                        <div class="date_of_birth_box experiencedate">
                                            ...
                                            <input type="text" name="date" class="datepicker" id="datepicker2" value="{{$method == 'Add' ? '': old('date',(@$result->getUserExperience->date == '' ? '' : date_format( date_create(@$result->getUserExperience->date), 'd M, Y' )))}}" autocomplete="off" readonly="readonly">
                                        </div>
                                    </div>  
                                </div>
                            </div>
                            <div class="form-group form_box">
                                <p>Description</p>
                                <textarea class="form-control form_textarea" name="description" id="description" placeholder="">{{old('description',@$result->getUserExperience->description)}}</textarea>
                            </div>
                            <div class="form-group form_box upload_photo">
