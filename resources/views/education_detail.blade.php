...
@section('pageContent')
    <div class="inner_back_center_row row">
        <div class="inner_back_col">
            <a href="{{ (Common_function::previous_url()[0][3] == strtolower(@$module)) ? route('education.index') : Common_function::previous_url()[1] }}" class="btn_back">
            ...
            <div class="education-imgs-lider slider">
                <div class="banner_box">
                    <div class="banner_img">                          
                        @php
                            @$ext = explode(".",@$results->getUserEducation->file);                             
                        @endphp
                        @if(@$results->getUserEducation->file != '')
                            @if(@$ext[1] == 'mp4' || @$ext[1] == 'ogv' || @$ext[1] == 'webm') 
                            <div class="video_box"></div>                           
                                <video controls class="img-fluid" disablepictureinpicture controlslist="nodownload"><source id="videoSource"/ src="{{@$results->getUserEducation->file ? Common_function::get_s3_file(config('custom_config.s3_education_video'),@$results->getUserEducation->file) : asset('images/default/education_default.jpg')}}"></video>
                            @else
                                <img src="{{ @$results->getUserEducation->file ? Common_function::get_s3_file(config('custom_config.s3_education_big'),@$results->getUserEducation->file) : asset('images/default/education_default.jpg') }}" class="img-fluid"
                                    alt="VioGraf" />
                            @endif
                        @else 
                            <img src="{{ asset('images/default/education_default.jpg') }}" class="img-fluid" alt="VioGraf" />   
                        @endif 
            ...
            <div class="post-detail-area">
                <div class="post_title_row">
                    <div class="post_title_box">
                        <h1 class="main_title">{{(@$results->getUserEducation->name != '') ? @$results->getUserEducation->name : '-'}}
                            <i>
                            @if (@$results->getUserEducation->start_date == '' && @$results->getUserEducation->end_date == '')
                                -
                            @else
                                @if (@$results->getUserEducation->start_date != '' && @$results->getUserEducation->end_date != '')
                                    {{date(' M Y',strtotime(@$results->getUserEducation->start_date)) .' - '.date(' M Y',strtotime(@$results->getUserEducation->end_date))}}
                                @else
                                    {{date(' M Y',strtotime(@$results->getUserEducation->start_date)) .' - '. 'Pursuing'}}
                                @endif
                            @endif
                            </i>
                        </h1>
                    ...
                    @if(@$results->user_id == Auth::user()->id)
                        <div class="edit_delete_icon">
                            <a href="{{url('/education/edit/'.@$results->id)}}" data-toggle="tooltip" title="Edit">
                ...
                <div class="post_detail_box">               
                    <div class="rank_predovic">
                        <h4>
                            ...
                            </span> {{ (@$results->getUserEducation->achievements != '') ? @$results->getUserEducation->achievements  : '-' }}
                        </h4>
                        <h4>
                            <span> 
                            <img src="{{asset ('images/icons/predovic.svg') }}" class="img-fluid" alt="VioGraf"/> 
                            </span> {{ (@$results->getUserEducation->my_buddies != '') ? @$results->getUserEducation->my_buddies : '-' }}
                        </h4>
                    </div>                  
                    ...
                    <div class="detail_info">
                        <h3>Description</h3>        
                        <h4><span class="desc-info">{!!(@$results->getUserEducation->description != '') ? Common_function::string_read_more(@$results->getUserEducation->description) : '-' !!}</span></h4>                   
                    </div>
                </div>
                @if(@$results->is_save_as_draft == 0)
                    <div>
                        <ul class="like_comments_row">
                            <li class="like_box">
                                <button type="button" class="like_unlike {{(@$results->getUserEducationLike->is_like != 0 )? 'active':''}}" id="like_unlike" value=""><i class="fa fa-heart" aria-hidden="true"></i>
                    ...
                    <div class="comment_area">
                        ...
                        <ul class="comment_list">
                            <div class="cmts">
                                @foreach (@$results->getUserEducationComment as $k => $val)
                                    <li>
                                        <div class="img_col">
                                            <img src="{{  @$val->getEducationCommentedUser->profile_image ? Common_function::get_s3_file(config('custom_config.s3_user_big'),  @$val->getEducationCommentedUser->profile_image) : url('images/userdefault-1.svg') }}" alt="Family Feed" class="img-fluid">
                                        </div>
                                        <a href="{{route('public-profile.index',@$val->getEducationCommentedUser->id)}}">
                                            <div class="commentdetail">
                                                <h4>{{@$val->getEducationCommentedUser->first_name.' '.@$val->getEducationCommentedUser->last_name}}  <p class="commentTime">&nbsp;{{Common_function::get_time_ago(strtotime($val->created_at))}}</p></h4>
                                                <p>{{@$val->comment}}</p>
                                            </div>
                                        </a>
                                    </li>
                            ...
                            @if(@$results->comment_count > 3)
                                <div class="view_more_commentsarea" >
                                    <div id="view_more_commentsarea">
                                        <button class="btn-more" id="btn-more" data-cmtcount="{{count(@$results->getUserEducationComment)}}" data-id ="{{@$val->id}}">View more comments</button>
                                    </div>
                                    <span> {{count(@$results->getUserEducationComment)}} of {{Common_function::like_comment_count(@$results->comment_count)}}</span>
                                </div>
                            @endif
                        </ul>
                    </div>
